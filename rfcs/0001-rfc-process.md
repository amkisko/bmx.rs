...
This RFC defines how bmx design changes are proposed, written, numbered, and advanced.

## Motivation

The `bmx` command family, `~/.bmx` state layout, trust policy, isolation defaults, and integrity_check fail-closed behavior are the public contract. SPEC.md is large enough that a pull request description cannot review a trust-schema change next to a new command. Numbered RFCs are the unit before those files move. RFC 0002 records the install-from-source problems and the limits this version still has.
