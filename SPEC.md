...
Commands: `bmx <app> [-- …]` (optional `--rm`, `--pin`, `--trust`, `--global`, `-v` / `--verbose`); `bmx exec <app> [-- …]` or `bmx exec --pin` when `.bmx/pins.toml` (or legacy `.bmx/pin`) defines the app; `bmx install [--as NAME]|rebuild [--install] [APP]|uninstall|reinstall|self-update|update`; `bmx show <app> [--would-remove]`; `bmx search [WORDS …] [--backend auto|github|gitlab|aur|homebrew] [-n N] [--forks] [--url] [--no-probe]`; `bmx history [-n N]`; `bmx undo [ID] [--only APP]`; `bmx source set-default|show`; `bmx isolation set-default|show`; `bmx checkout set-default|show`; `bmx trust list|show|add-key|set-signed|set-allow|import-repo|check`; `bmx shim init|path|add`; `bmx doctor`.
